...
func (f *fakeConsul) wait(ctx context.Context, clientWaitIndex uint64, waitStr string) {
	if clientWaitIndex == 0 || f.index > clientWaitIndex {
		return
	}

	waitDur := parseDurationOrZero(waitStr)
	if waitDur <= 0 {
...
func (f *fakeConsul) handle(w http.ResponseWriter, r *http.Request) {
	...
	f.wait(ctx, clientWaitIndex, q.Get("wait"))
	...
	f.writeSuccessResponse(w, r.URL.Path, idx, value)
}

func (f *fakeConsul) writeSuccessResponse(w http.ResponseWriter, path string, idx uint64, value []byte) {
	pair := struct {
		Key         string `json:"Key"`
